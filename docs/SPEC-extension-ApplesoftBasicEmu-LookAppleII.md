...
|---|---|---|
| RG-FID-0001 | 1b | Le pavé clignotant est un caractère espace (`&nbsp;`) styled inverse-vidéo (fond vert phosphore plein, hauteur d'un caractère, largeur de `1ch` via héritage des métriques de la police bitmap Apple II), animé en CSS via une bascule `background-color` toutes les 500 ms (clignotement à ~2 Hz, conforme au curseur Apple II d'origine). |
...
- **CA-UC-FID-001-01 :** Soit l'émulateur web chargé et le prompt `]` affiché, Quand aucune frappe n'a été faite, Alors un pavé bloc inverse-vidéo vert de la largeur d'un caractère apparaît immédiatement à droite du `]` et clignote à 2 Hz.
- **CA-UC-FID-001-02 :** Soit le prompt `]` avec le pavé clignotant à droite, Quand l'utilisateur tape la lettre `H`, Alors `H` apparaît entre le `]` et le pavé clignotant, le pavé restant immédiatement à droite du `H`.
...
| Terme | Définition |
|---|---|
| Pavé clignotant | Bloc inverse-vidéo de la dimension d'un caractère, alternant entre fond plein vert phosphore et fond transparent à environ 2 Hz. Modélise le curseur de saisie de l'Apple II d'origine. |
