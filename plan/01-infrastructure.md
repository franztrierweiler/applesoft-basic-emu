...
Mettre en place la structure du projet, l'outillage de développement et le pipeline CI/CD. À la fin de cet EPIC, un développeur (ou agent IA) peut cloner le dépôt, installer les dépendances dev, lancer les tests et le linter sans erreur.
...
| 1.4 | Créer le `Makefile` avec les cibles : `help`, `install`, `test`, `lint`, `run`, `web-build`, `web-serve`, `clean` | ✅ |
| 1.5 | Créer le workflow GitHub Actions (`.github/workflows/ci.yml`) : lint + tests sur push/PR | ✅ |
| 1.6 | Configurer ruff (section `pyproject.toml`) et mypy | ✅ |
| 1.7 | Créer `tests/conftest.py` avec fixtures de base (IOBridge mock) | ✅ |
| 1.8 | Vérifier que `make install && make lint && make test` passent sans erreur | ✅ |
